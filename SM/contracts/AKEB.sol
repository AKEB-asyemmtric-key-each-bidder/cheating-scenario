...
contract AKEB {
    ...
    modifier checkIfAuctionIsStarted() {
        require(isAuctionStarted == true, "There is no started auction now");
        _;
    }
    ...
    modifier onlyBidders() {
        require(
            isBidder() == true,
            "Only registerd bidder can call this function"
        );
        _;
    }
    ...
    function setUpPhasesTimePeriods() private {
        ...
        winnerAndDisputeSubmissionPeriod =
            block.timestamp +
            (3 * periodTime * 60);
    }

    // Registering auction infomation such as name and description of the asset
    function registerAuctionInfo(
        string memory assetNameInput,
    ...
    function registerBidder() public checkIfAuctionIsStarted {
        require(msg.sender != seller, "Seller can not register as bidder");
        require(
            block.timestamp < registerBidderPeriod,
            "Time for registering as a bidder is passed."
        );
    ...
    function dispute(
        ...
        bytes32 disputerHash = computeHash(
            inputDisputedBid,
            inputDisputedNonce
        );

        require(
            encodedBids[msg.sender] == disputerHash,
            "The bid and nonce you submitted are not valid."
        );
        require(
            isDiputeBidGreaterThanOneWinnerBid(inputDisputedBid) == true,
            "Your dispute is not valid since it is not greater than at least one winner."
        );
    ...
    function reset() public onlyAuctioneer {
        require(
            block.timestamp > winnerAndDisputeSubmissionPeriod,
    ...
    function resetEncodedBids() public onlyAuctioneer {
        require(
            block.timestamp > winnerAndDisputeSubmissionPeriod,
            "Auction is not completed yet."
        );
    ...
    function addWinner(uint256 bid, string memory nonce) private {
        winner memory temp = winner(msg.sender, bid, nonce);

        winners.push(temp);
    }

    function isDiputeBidGreaterThanOneWinnerBid(
        uint256 disputeBid
    ...
    function computeHash(
        ...
        string memory bidAndNonceConcatenation = string.concat(
            bidInString,
            nonce
        );
